class Game {
  constructor(theme, difficulty) {
    this.theme=theme;
    this.difficulty=[[5,2],[6,3],[8,3]][+difficulty];
  ...
  createBoard() {
    ...
    if(this.theme==="2")
      imageAmount = 12;
    let cardArray=[];
    let cardAmount=this.difficulty[0]*this.difficulty[1];
    ...
    for(let i=0; i<cardAmount; i++) {
      this.board.innerHTML+='<div class="card">\
      <figure class="front" >\
      <img src="images/theme'+this.theme+'.jpg" alt="front-card"/>\
      </figure>\
      <figure class="back" >\
      <img src="images/theme'+this.theme+'-'+cardArray[i]+'.jpg"alt="back-card"/>\
      </figure>\
      </div>';
    }
  ...
  clickCard(evt) {
    ...
    if(card.classList.contains('card')&&!card.classList.contains('clicked')&&this.openCount!==2) {
      this.openCount++;
  ...
  tryMatch() {
    let srcFirst=this.openedCards[0].querySelector('.back>img').src.slice(-6);
    let srcSecond=this.openedCards[1].querySelector('.back>img').src.slice(-6);
